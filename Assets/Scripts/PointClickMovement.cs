        ...
        if (Input.GetMouseButton(0) && !EventSystem.current.IsPointerOverGameObject()) // tet mission point by click
        {
            ...
            if (Physics.Raycast(ray, out mouseHit))
            {
                ...
                if (hitObject.layer == LayerMask.NameToLayer("Ground")) {
                    _targetPos = mouseHit.point; // set mission in point of hit
                    _curSpeed = moveSpeed;
                }
            }
        }

        if (_targetPos != Vector3.one) // if have a mission
        {
            Vector3 adjustedPos = new Vector3(_targetPos.x, transform.position.y, _targetPos.z);
            Quaternion targetRot = Quaternion.LookRotation(
                    adjustedPos - transform.position
                );
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRot, rotSpeed * Time.deltaTime); //rotate player face to mission
